...
def similarity(a, b):
    ...
    s = 0
    for i in range(0,M):
        s += ((a.iloc[i] - m_a)*(b.iloc[i] - m_b))
    s/= (M*np.std(a)*np.std(b))
    return s
...
def computeDelta(wt, X, Xi):
    ...
    for i in range(0,len(Xi)):
        x_i = Xi.iloc[i]
        temp = np.exp(wt*similarity(x, (x_i.iloc[0:(len(x_i)-1)])))
        den+= temp
        num+= x_i[-1]*temp
    return num/den
...
for i in range(0,len(train1_90.index)) :
  testDeltaP90 = np.append(testDeltaP90, computeDelta(2,test_90.iloc[i],train1_90))
for i in range(0,len(train1_180.index)) :
  testDeltaP180 = np.append(testDeltaP180, computeDelta(2,test_180.iloc[i],train1_180))
for i in range(0,len(train1_360.index)) :
  testDeltaP360 = np.append(testDeltaP360, computeDelta(2,test_360.iloc[i],train1_360))
